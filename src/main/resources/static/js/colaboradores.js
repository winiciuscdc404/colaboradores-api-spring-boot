...
function renderTabela() {
    ...
    const isAdmin = getRoleDoToken() === 'ADMIN';
    ...
    // Esconde o botão de novo colaborador se não for admin
...
async function salvar() {
    try {
        ...
        if (!modoEdicao && !senha) { alert('Informe a senha'); return; }
...
async function adicionarPontos(id) {
    ...
    if (!pontos || pontos <= 0) { alert('Informe um valor válido'); return; }
